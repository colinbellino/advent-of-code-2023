...
@(test) day_03a_test :: proc(t: ^testing.T) {
...
@(test) day_03b_test :: proc(t: ^testing.T) {
    context.logger = log.create_console_logger(.Debug, { .Level, .Terminal_Color })
    testing.expect_value(t, day_03b_process(aoc.load_file_or_fail(t, "day_03/day_03b_input_01.txt")), 467835)
    testing.expect_value(t, day_03b_process(aoc.load_file_or_fail(t, "day_03/day_03a_input_02.txt")), 80179647)
}
...
day_03a_process :: proc(input: []byte) -> (result: int) {
    ...
    is_valid :: proc(position: Vector2i32, data: string, grid_size: Vector2i32) -> bool {
        for direction in EIGHT_DIRECTIONS {
            ...
            if is_digit(neighbour) == false && neighbour != '.' {
                return true
            }
        }
        return false
...
day_03b_process :: proc(input: []byte) -> (result: int) {
    input_string := strings.clone_from_bytes(input)
    ...
    part_number := 0
    part_is_valid := false
    gears := map[int][dynamic]int {}
    gear := -1
    for char, i in input_flat {
        position := Vector2i32 { x, y }
        if is_digit(char) {
            part_number = concatenate_int(part_number, int(char - '0'))
            gear_index := get_gear(position, input_flat, grid_size)
            if gear_index != -1 {
                part_is_valid = true
                gear = gear_index
            }
            ...
            if part_is_valid {
                if gear != -1 {
                    if gear in gears {
                        append(&gears[gear], part_number)
                    } else {
                        gears[gear] = [dynamic]int { part_number }
                    }
                }
                // log.debugf("part: %v -> \"%v\" (%v)", position, part_number, gears)
            }
    ...
    get_gear :: proc(position: Vector2i32, data: string, grid_size: Vector2i32) -> int {
        for direction in EIGHT_DIRECTIONS {
            neighbour_position := position + direction
            if grid_position_is_in_bounds(neighbour_position, grid_size) == false {
                continue
